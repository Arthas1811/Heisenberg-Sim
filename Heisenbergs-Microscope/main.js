var HeisenbergMicroscope = (() => {
  ...
  function drawProjection() {
    ...
    ctx.stroke();
    const angle = 0; // horizontal to the right
    const ah = 10;
    ctx.beginPath();
  ...
  canvas.addEventListener("wheel", (e2) => {
    ...
    const zoomFactor = Math.exp(-e2.deltaY * 0.001);
    const newScale = clamp(camera.scale * zoomFactor, 0.4, 6.0);
